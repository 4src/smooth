...
-------------------------------------------------
-- function ABCD:new(b4,klass)
--   return {klass=klass, a=b4, b=0, c=0, d=0} end
...
-- function ABDC:f()         p,r=self:precision(),self:recall(); return (2*p*r)/(p+r) end
-- function ABDC:g()         nf,r=1-self:pf(),self:recall(); return (2*nf*r)/(nf+r) end
...
return {the=the, help=help, SYM=SYM, NUM=NUM,
        COLS=COLS, ROW=ROW, DATA=DATA, NB=NB}